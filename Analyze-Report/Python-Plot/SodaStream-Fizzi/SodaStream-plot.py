...
# Config
input_file = "SodaStream-All-Models-For-Analyze.xlsx"
# Set to 'mean', 'median', 'both', or 'none' (default: mean)
summary_measure = "mean"

# Global x-axis limits (will be applied to every subplot)
# Change these to adjust the visible range; out-of-range means/medians
# will be marked at the axis edge instead of drawn off-figure.
X_MIN, X_MAX = 0, 30
...
fig, axes = plt.subplots(nrows=n, ncols=1, sharex=True, figsize=(12, fig_height))
if n == 1:
    axes = [axes]
...
for i, model in enumerate(models):
    ...
    # Draw mean/median only if inside the visible x-range; otherwise mark edge
    if summary_measure in ("mean", "both") and mean_val is not None:
        if X_MIN <= mean_val <= X_MAX:
            ...
            # place a short marker and label just inside the axis edge
            if mean_val > X_MAX:
                edge_x = X_MAX - (X_MAX - X_MIN) * 0.02
                ha = "right"
                label = f">{int(X_MAX)} ({mean_val:.1f})"
            else:
                edge_x = X_MIN + (X_MAX - X_MIN) * 0.02
                ha = "left"
                label = f"<{int(X_MIN)} ({mean_val:.1f})"
            ax.axvline(edge_x, color="navy", linestyle="--", linewidth=1.0)
            ax.text(edge_x, ymax * 0.9, label, rotation=90, va="top", ha=ha, fontsize=8, color="navy")
    if summary_measure in ("median", "both") and median_val is not None:
        if X_MIN <= median_val <= X_MAX:
            ...
            if median_val > X_MAX:
                edge_x = X_MAX - (X_MAX - X_MIN) * 0.02
                ha = "right"
                label = f">{int(X_MAX)} ({median_val:.1f})"
            else:
                edge_x = X_MIN + (X_MAX - X_MIN) * 0.02
                ha = "left"
                label = f"<{int(X_MIN)} ({median_val:.1f})"
            ax.axvline(edge_x, color="darkgreen", linestyle=":", linewidth=1.0)
            ax.text(edge_x, ymax * 0.75, label, rotation=90, va="top", ha=ha, fontsize=8, color="darkgreen")
...
fig.suptitle("Evaluation of 12 Language Models' Predictions of SodaStream Fizzi Carbon Footprint (kg CO₂e)", fontsize=12)
plt.tight_layout()

# Make room for the top label then set global x-limits
# ensure axes expand to use most of the figure width
fig.subplots_adjust(top=0.92, left=0.08, right=0.99)
...
output_png = f"sodastream_models_rows_{tag}.png"
